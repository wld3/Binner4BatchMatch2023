...
public class BinnerNonAnnotatedFeatureWriter extends BinnerSpreadSheetWriter implements Serializable {
	private static final long serialVersionUID = -118116585712037767L;
	private SXSSFWorkbook workBook;
	private int rowCt;
	private int rowIndex;
	
	public BinnerNonAnnotatedFeatureWriter(SXSSFWorkbook workBook)  {
		super();
		this.workBook = workBook;
	}
	
	public void createFeatureTab(AnalysisData analysisData, BinnerOutput output, List<XSSFCellStyle> styleList, Boolean onlyRebin ) throws Exception {
		...
		sheet.setColumnWidth(BinnerConstants.ISO_GROUP_COL, 30 * 256);
		sheet.setColumnWidth(BinnerConstants.ANNOTATION_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.DERIVATION_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.MASS_ERR_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.FEAT_GROUP_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.CHARGE_CARR_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.NEUT_ANNOT_COL, 256); 
		sheet.setColumnWidth(BinnerConstants.REF_MASS_COL, 256); 
		...
		createHeader(output, sheet, styleList);
		for (BinnerGroup group : output.getGroups())
			createFeatureList(sheet, analysisData, group, styleBoring, styleInteger, styleNumeric, styleBoringLeft, styleList, onlyRebin);
	}
		
	
	public void createFeatureList(Sheet sheet, AnalysisData analysisData, BinnerGroup group, XSSFCellStyle styleBoring,
		...
		for (int j = featureIndices.size() - 1; j >= 0; j--)  {
			...
			if (!StringUtils.isEmptyOrNull(feature.getAdductOrNL()))
				continue;
			
			if (!StringUtils.isEmptyOrNull(feature.getIsotope()) && feature.getIsotope().contains("+"))
				continue;
	...
	private void createIntensitySection(Feature feature, Sheet sheet, int i, List<XSSFCellStyle> styleList) {
		...
		for (Double dblValue : valueList) {
			if (outlierMap.containsKey(sampleIdx))
				...
				createAppropriateRowEntry(rowCt, i++, sheet, ".", styleList.get(BinnerConstants.STYLE_INTEGER_GREY_CENTERED),
	...
	public void createHeader(BinnerOutput output, Sheet sheet,  List<XSSFCellStyle> styleList) throws Exception {
		rowCt = 0;
		rowIndex = 1;
		int col = 0;
		sheet.createRow(rowCt);
